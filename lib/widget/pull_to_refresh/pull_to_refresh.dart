...
import 'package:test_app/widget/pull_to_refresh/proxy_header_widget.dart';
...
class _PullRefreshState extends  State<PullToRefreshWidget> {
  ...
  /// 下拉刷新的刷新状态
  RefreshStatus _headerState = RefreshStatus.reset;
  ...
  bool _handleScrollUpdate(ScrollUpdateNotification notification) {
    ...
    /// 已经处于正在刷新状态，就直接返回
    if (_headerState == RefreshStatus.refreshing) {
      return false;
    }
    ...
    if (_headerState == RefreshStatus.pullToRefresh &&
        offset >= _maxDistance) {
      _headerState = RefreshStatus.releaseToRefresh;
    } else if (_headerState != RefreshStatus.pullToRefresh &&
        offset < _maxDistance) {
      _headerState = RefreshStatus.pullToRefresh;
    }
    return false;
  }
  ...
  bool _handleScrollEnd(ScrollNotification notification) {
    ...
    /// 松开后刷新
    if (_headerState == RefreshStatus.releaseToRefresh &&
        widget.onRefresh != null) {
      _headerState = RefreshStatus.refreshing;
      callback.refreshing();
      ...
      refreshResult.whenComplete(() {
        _headerState = RefreshStatus.reset;
        callback.reset();
      });
    ...
    if (_headerState == RefreshStatus.refreshing) return false;
    callback.reset();
    return false;
  }
